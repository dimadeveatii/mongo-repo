...
Repository.prototype.addOrUpdate = function (documentId, document) {
  ...
  return this.addOrUpdateOne(searchByIdCriteria(documentId), document)
}

Repository.prototype.addOrUpdateOne = function (criteria, document) {
  if (!document) return when.reject(new Error('Missing parameter criteria.'))
  if (!document) return when.reject(new Error('Missing parameter document.'))
